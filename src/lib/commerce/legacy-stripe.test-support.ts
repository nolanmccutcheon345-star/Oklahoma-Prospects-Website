/** Offline regression fixtures for historical records. Never imported by the application. */
...
export async function fulfillCheckout(tx: Sql, session: Stripe.Checkout.Session) {
  ...
  if (!order || session.amount_total !== order.total_cents || session.currency !== "usd")
  ...
  if (["paid", "refunded", "cancelled", "refunding", "payment_review"].includes(order.status))
    return;
  const bookings = await tx<{
    id: string;
    status: string;
  }>`select id,status from booking_records where order_id = ${id}`;
  ...
  if (!order.snapshot.recurring && order.snapshot.kind === "package") {
    ...
    const end = new Date(start.getTime() + order.snapshot.expiresDays * 86_400_000);
...
export async function processStripeEvent(
  event: Stripe.Event,
  dependencies: { stripe: Stripe; sql: Sql; mode: "test" | "live" },
) {
  const stripe = dependencies.stripe;
  if (event.livemode !== (dependencies.mode === "live"))
  ...
  if (
    ...
    await sql.transaction(async (tx) => {
      const inserted =
        await tx`insert into stripe_events (id,type) values (${event.id},${event.type}) on conflict do nothing returning id`;
      if (!inserted.length) return;
      if (current.subscription && typeof current.subscription === "object")
        await storeSubscription(tx, current.subscription);
  ...
  if (event.type === "invoice.paid" || event.type === "invoice.payment_failed") {
    ...
    await sql.transaction(async (tx) => {
      ...
      const line =
        invoice.lines.data.find((l) => l.parent?.subscription_item_details) ||
        invoice.lines.data[0];
      if (!line) throw new Error("Invoice period is missing.");
      const start = instant(line.period.start),
        end = instant(line.period.end);
      ...
      if (invoice.amount_paid !== expected)
        throw new Error("Invoice amount does not match this membership.");
      if (["refunded", "cancelled", "refunding", "payment_review"].includes(order.status)) return;
      if (!initial && ["m1", "m2", "m3"].includes(order.product_id))
        await carryOneSession(tx, subscription.id, start, end);
      ...
      await grantCredits(tx, {
        ...
        initialBooking: initial && Boolean(booking),
      });
    });
    return;
  }
  ...
  if (
    ...
    await sql.transaction(async (tx) => {
      ...
      const [row] = await tx<{
        order_id: string;
      }>`update commerce_refunds set status=${refund.status || "pending"} where stripe_refund_id=${refund.id} returning order_id`;
      if (row && refund.status === "succeeded")
        await tx`update commerce_orders set status=case when total_cents=${refund.amount} then 'refunded' else 'cancelled' end where id=${row.order_id}`;
      if (row && (refund.status === "failed" || refund.status === "canceled"))
        await tx`insert into club_requests(id,user_id,kind,payload) values(${"refund-failed:" + refund.id},null,'refund-failed',${JSON.stringify({ orderId: row.order_id, refundId: refund.id })}::jsonb) on conflict do nothing`;
    });
    return;
  }
  if (event.type === "charge.refunded") {
    ...
    await sql.transaction(async (tx) => {
      ...
      const orders = await tx<{
        id: string;
      }>`update commerce_orders set status='refunded' where payment_intent_id=${stripeId(charge.payment_intent)} returning id`;
  ...
  if (
    ...
    await sql.transaction(async (tx) => {
      ...
      const [order] = await tx<{
        id: string;
      }>`update commerce_orders set status = 'failed' where checkout_session_id = ${session.id} and status = 'pending' returning id`;
      if (order) {
...
export async function prepareCancellation(
  ...
  if (order.snapshot.recurring || order.snapshot.kind === "package")
    throw new Error("This purchase needs front-office review.");
  ...
  if (amount > 0 && !order.payment_intent_id)
    throw new Error("Your payment needs front-office review before refunding.");
