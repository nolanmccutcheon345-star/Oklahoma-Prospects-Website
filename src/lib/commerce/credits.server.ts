...
async function context(userId:string,input:Input) {
  ...
  if(grant.kind==='cage-minutes') {
    ...
    if(!input.duration||!BOOKABLE_LANES.some(l=>l.id===input.laneId&&l.group!=='field'))throw new Error('Choose a cage and duration. The fielding area is billed separately at its listed rate.');
    minutes=input.duration;quantity=minutes;resources=[`lane:${input.laneId}`];
  } else {
    const [assessment]=await sql`select id from athlete_assessments where athlete_id=${grant.athlete_id} limit 1`;
    if(!assessment)throw new Error('Complete your assessment with your coach before booking ordinary lessons.');
    if(grant.product_id==='m4')throw new Error('Small-group credits require a published group session. Contact your coach for the weekly group schedule.');
    const [service]=await sql<Product>`select * from club_services where id=${input.serviceId} and active=true`;
...
export async function creditSlots(userId:string,input:Input) {
  ...
  if(grant.kind==='remote-review')return [];
  ...
  return slotsFor(input.date,minutes).filter(s=>{
    ...
    return end<=new Date(grant.expires_at)&&start>=new Date(grant.starts_at)
      && (grant.kind==='cage-minutes'||coachAvailable(file.availability,input.coachId!,input.date,s.value,minutes))
...
export async function redeemCredit(userId:string,input:Input) {
  ...
  return sql.transaction(async tx=>{
    ...
    if(grant.kind==='remote-review') {
      ...
      await tx`insert into club_requests(id,user_id,kind,payload) values(${randomUUID()},${userId},'video-review',${JSON.stringify({athleteId:grant.athlete_id,coachId:input.coachId,videoUrl:input.videoUrl,grantId:grant.id})}::jsonb)`;
    } else {
