...
export async function deliverPaymentNotifications(
  ...
  const notices = await sql<{
    ...
    order by case when n.kind like 'owner-%' then 0 else 1 end,n.created_at limit ${ownerTest ? 1 : 4}`;
  ...
  await Promise.all(
    notices.map(async (n) => {
      ...
      try {
        ...
        // Keep this notice pending and return; a failed customer email must not block owners.
