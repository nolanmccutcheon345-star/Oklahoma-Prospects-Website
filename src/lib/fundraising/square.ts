...
export async function syncPayment(paymentId: string, deps = { square, db, config }) {
  const { payment: p } = await deps.square("/payments/" + encodeURIComponent(paymentId));
  ...
  const row = await deps
    ...
    .first<any>();
  if (!row) return;
  const checked = validatePayment(p, row, deps.config().SQUARE_LOCATION_ID);
  const receipt =
  ...
  await deps
    .db()
    .prepare(
      "UPDATE fundraising_contributions SET status=CASE WHEN status='completed' THEN status ELSE $1 END,refunded=GREATEST(refunded,$2),payment_id=$3,receipt_url=$4,checked=$5 WHERE id=$6 AND (status<>'completed' OR payment_id IS NULL OR payment_id=$7)",
    )
    .bind(checked.status, checked.refunded, p.id, receipt, new Date().toISOString(), row.id, p.id)
    .run();
}
export async function syncContribution(id: string, deps = { square, db, config }) {
  const row = await deps
    .db()
    ...
    .first<any>();
  if (!row) return null;
  if (row.payment_id) await syncPayment(row.payment_id, deps);
  // A failed attempt must not hide a later successful tender for the same order.
  if (row.order_id && row.status !== "completed") {
    const { order } = await deps.square("/orders/" + encodeURIComponent(row.order_id));
    if (
      order?.id !== row.order_id ||
      order?.reference_id !== row.id ||
      order?.location_id !== deps.config().SQUARE_LOCATION_ID
    )
      throw new AppError("Order verification failed.", 409);
    for (const t of order.tenders || []) if (t.payment_id) await syncPayment(t.payment_id, deps);
  }
  return deps
    .db()
    .prepare(
