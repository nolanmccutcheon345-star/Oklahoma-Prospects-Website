...
export const getHouseholds=createServerFn({method:'GET'}).middleware([authMiddleware]).handler(async({context})=>{const m=await import('./invitations.server');return m.myHouseholds(context.userId);});
