...
import { getFamilyBilling, claimPurchases, stopAutoRenew, openBillingPortal, askForPause, getRefundPreview, requestRefund, checkInBooking } from "@/lib/commerce/portal-api";
...
export function FamilyBilling({bookingsOnly=false}:{bookingsOnly?:boolean}) {
  ...
  async function load(){setData(await getFamilyBilling());}
  useEffect(()=>{if(user) void load().catch(e=>setError(e instanceof Error?e.message:"Billing could not load."));},[user?.id]);
  ...
  return <section className="my-6 grid gap-5" aria-label="Bookings and billing">
    ...
    {data?.bookings.map(b=><article key={b.id} className="rounded-xl border p-4"><h3 className="text-xl">{data.orders.find(o=>o.id===b.order_id)?.snapshot.title || "Training session"}</h3><p>{dateText(b.starts_at)} · {b.status}</p>
